...
def equal(A, B):
    ...
    for r in A.D[0]:
        for c in A.D[1]:
            if A.f.get((r,c),0) != B.f.get((r,c),0):
                Res = False
                break
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {(i,j):A.f.get((i,j),0)+B.f.get((i,j),0) for i in A.D[0] for j in A.D[1]})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {(i,j): x * M.f.get((i,j),0) for i in M.D[0] for j in M.D[1]})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(j,i): M.f.get((i,j),0) for i in M.D[0] for j in M.D[1]})
...
def vector_matrix_mul(v, M):
    ...
    return Vec(M.D[1], {i:sum(v.f.get(j,0) * M.f.get((j,i),0) for j in v.D) for i in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    return Vec(M.D[0], {i: sum(M.f.get((i,j),0) * v.f.get(j,0) for j in v.D) for i in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    return Mat((A.D[0],B.D[1]), {(r1,r2): sum(A.f.get((r1,i),0)* B.f.get((i,r2),0) for i in A.D[1]) for r1 in A.D[0] for r2 in B.D[1]})
